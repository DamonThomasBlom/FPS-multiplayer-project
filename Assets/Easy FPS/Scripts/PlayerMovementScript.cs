...
public class PlayerMovementScript : MonoBehaviour {
	...
	*/
	void FixedUpdate(){
        ...
        //if (!(Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.WindowsEditor))  prevoius code
        if (platform_manager.curPlatform == platform.windows)
           
            PlayerMovementLogic(new Vector2(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical")));
	...
	*/
	public void PlayerMovementLogic(Vector2 input){
            ...
            if (grounded)
            {
                rb.AddRelativeForce(input.x * accelerationSpeed * Time.deltaTime, 0, input.y * accelerationSpeed * Time.deltaTime);
            }
            else
            {
                rb.AddRelativeForce(input.x * accelerationSpeed / 2 * Time.deltaTime, 0, input.y * accelerationSpeed / 2 * Time.deltaTime);
