	...
	*/
	void  Update(){
        ...
        if (platform_manager.curPlatform == platform.windows)
            MouseInputMovement(new Vector2(Input.GetAxis("Mouse X"),Input.GetAxis("Mouse Y")));
        ...
        if (_pauseMenu.pauseMenuIsActive)
        {
            Cursor.visible = true;
        }
        else if (_pauseMenu.pauseMenuIsActive == false)
        {
 ...
 */
public void MouseInputMovement(Vector2 touchLook){

	wantedYRotation += touchLook.x * mouseSensitvity;

	wantedCameraXRotation -= touchLook.y * mouseSensitvity;
     ...
     */
    void ApplyingStuff(GameObject playerGo){
	...
	playerGo.transform.rotation = Quaternion.Euler(0, currentYRotation, 0);
