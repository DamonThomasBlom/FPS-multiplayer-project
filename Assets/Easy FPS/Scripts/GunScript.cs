...
public class GunScript : MonoBehaviour {
    ...
    void Awake(){
       ...
       UIManager uiManager = FindObjectOfType<UIManager>();
        ...
        if (uiManager != null)
        {
            uiManager.FireButton.OnDown.AddListener(ShootMethod);
            uiManager.AimButton.OnDown.AddListener(()=> { isAiming = true; });
            uiManager.AimButton.OnUp.AddListener(() => { isAiming = false; });
        }
...
void FixedUpdate()
    {
        ...
        if ((Input.GetAxis("Fire2") != 0 && !reloading && !meeleAttack && platform_manager.curPlatform != platform.android) | isAiming)
        {
            AimWeapon();
        }
        //if not aiming
	...
	*/
	...
	void Shooting(){
		...
		if (!meeleAttack) {
			if (currentStyle == GunStyles.nonautomatic) {
                ...
                //if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.WindowsEditor)   previous code
	...
	public float cameraZoomRatio_notAiming = 70;
	...
	private void ShootMethod(){
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5){
			...
			if(bulletsInTheGun > 0){
                ...
                if (bullet)
                {
                    GameObject firedBullet = Instantiate(bullet, bulletSpawnPlace.transform.position, bulletSpawnPlace.transform.rotation);
                    firedBullet.GetComponent<BulletScript>().owner = PhotonNetwork.NickName;
